...
uncleaned_text = pd.read_csv("values/check.csv")
def clean_html(text):
...
extracted_data = [
  ...
  for _, row in uncleaned_text.iterrows()
]
  ...
  for prompt in prompts:
    ...
    for message in messages:
      completion = client.chat.completions.create(
        model="deepseek-chat", #Change this depending on the model
        messages=[
          {
      ...
      responses.append(response_json)
      print()
    with open(f"results/moe/deepseek_v3_{t}_run_{i}_rerun.json", "w") as outfile:
